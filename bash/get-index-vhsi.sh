...
echo $SEC_FILE
wget -nv -O $LOGDIR/"$SEC_FILE" -w 1 --random-wait -e robots=off -T 60 -U "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:47.0) Gecko/20100101 Firefox/47.0" "http://www.hsi.com.hk/HSI-Net/static/revamp/contents/en/indexes/report/vhsi/$SEC_FILE_OLD"
...
cat $LOGDIR/$SEC_FILE.idx.tmp | cut -d"," -f2,7 --complement > $LOGDIR/$SEC_FILE.idx
sed -i 's/^/vhsi,/g' $LOGDIR/$SEC_FILE.idx

sed -i "1i code,date,high,low,close,change" $LOGDIR/$SEC_FILE.idx
